 ...
 * 但是这样在后续的判定中使情况变得复杂多了，因此一直卡在这里。而答案中栈直接存的就是每一层的分数。另一个卡点就是不知道
 ...
 * 第二次做：已经有把遇到左括号把0当作新score加入栈的思想，但仍有想着把左括号和score一起入栈。另一个卡点还是不知道遇到右括号弹栈后怎么计算分数，
 * 因为需要区分嵌套和并列的情况，以及用栈顶作为当前层的分数。从代码可知，在并列的情况时，score已经被计算过没有被置0，所以用max来取值。
 * */
