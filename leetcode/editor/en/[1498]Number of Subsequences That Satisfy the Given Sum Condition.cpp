...
public:
    int numSubseq(vector<int>& nums, int target) {
        ...
        long long MOD = 1e9 + 7;
        vector<int> cnt(nums.size()); // the number of permutation in each index
        cnt[0] = 1; // there is only one element at index = 0, so the permutation should 1
...
//            cnt[i] = (cnt[i - 1] << 1) % MOD;
        ...
        while (left <= right) { // left == right means there is one element inside the window
            int sum = nums[left] + nums[right];
            if (sum <= target) {
                res = (res + cnt[right - left]) % MOD;
                left++;
            }
            else right--;
        }
